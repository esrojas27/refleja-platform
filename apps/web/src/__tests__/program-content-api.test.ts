...
import { createProgramModule, createProgramSession, listProgramModules } from "@/lib/programs/program-content-api";
...
describe("program content API", () => {
  ...
  it("loads the tenant and program scoped module structure without cache", async () => {
    const fetcher = vi.fn().mockResolvedValue(new Response(JSON.stringify({ items: [] }), { status: 200 }));
    vi.stubGlobal("fetch", fetcher);
    const signal = new AbortController().signal;
    await listProgramModules("org a", "program/a", signal);
  ...
  it("sends only the module and session creation contracts", async () => {
    const fetcher = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: "module-a" }), { status: 201 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: "session-a" }), { status: 201 }));
    vi.stubGlobal("fetch", fetcher);
    const moduleInput = { name: "Inicio", description: "Contexto", position: 1 };
    const sessionInput = { name: "Sesión inicial", description: "Encuentro", scheduledDate: "2026-10-01", position: 1 };
    await createProgramModule("org-a", "program-a", moduleInput);
    await createProgramSession("org-a", "program-a", "module-a", sessionInput);
    expect(fetcher.mock.calls[0][1]).toEqual(expect.objectContaining({ method: "POST", body: JSON.stringify(moduleInput) }));
  ...
  it("does not call the backend without an access token", async () => {
    ...
    await expect(listProgramModules("org", "program")).rejects.toMatchObject({ status: 401 });
