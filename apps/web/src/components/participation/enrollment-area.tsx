  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setMessage(data.items.length ? "" : "No hay colaboradores en esta página.");
      } catch (error) {
...
export function EnrollmentForm({ organizationId, programId, onSaved }: {
  ...
  const [input, setInput] = useState<EnrollmentInput>({ email: "", firstName: "", lastName: "" });
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const clean = { email: input.email.trim(), firstName: input.firstName.trim(), lastName: input.lastName.trim() };
    const invalid = [];
    ...
    setFields(invalid);
    if (invalid.length) { setMessage("Revisa el correo, el nombre y el apellido."); return; }
    submitting.current = true; setPending(true); setMessage("Registrando colaborador…");
    const controller = new AbortController(); request.current = controller;
    try {
      const saved = await createEnrollment(organizationId, programId, clean, controller.signal);
      if (!controller.signal.aborted) { setBlocked(true); setMessage("Colaborador registrado."); onSaved(saved); }
    } catch (error) {
      if (controller.signal.aborted) return;
