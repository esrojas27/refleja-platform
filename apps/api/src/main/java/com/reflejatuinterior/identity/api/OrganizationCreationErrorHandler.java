...
@RestControllerAdvice(assignableTypes = OrganizationCreationController.class)
class OrganizationCreationErrorHandler {
    ...
    @ExceptionHandler(InvalidOrganizationInput.class)
    ResponseEntity<ErrorResponse> invalid(InvalidOrganizationInput exception, HttpServletRequest request) {
        return error(request, 400, "VALIDATION_ERROR", List.of(new FieldError(exception.field(), "INVALID_VALUE", "Invalid value.")));
    }
    ...
    ResponseEntity<ErrorResponse> conflict(HttpServletRequest request) {
        return error(request, 409, "ORGANIZATION_CREATION_CONFLICT", List.of());
    }
    ...
    private ResponseEntity<ErrorResponse> error(HttpServletRequest request, int status, String code, List<FieldError> fields) {
        ...
        String message = switch (status) {
            ...
            case 403 -> "Organization creation is not permitted.";
            case 409 -> "Organization creation conflicted with persisted state.";
