...
@RestControllerAdvice(assignableTypes = ProgramStructureController.class)
class ProgramStructureErrorHandler {
    ...
    ResponseEntity<ErrorResponse> missing(HttpServletRequest request) {
        return error(request, 404, "PROGRAM_NOT_FOUND", List.of());
    }
    ...
    ResponseEntity<ErrorResponse> invalid(InvalidProgramInput exception, HttpServletRequest request) {
        return error(request, 400, "VALIDATION_ERROR",
                List.of(new FieldError(exception.field(), "INVALID_VALUE", "Invalid value.")));
    }
    ...
    ResponseEntity<ErrorResponse> conflict(HttpServletRequest request) {
        return error(request, 409, "PROGRAM_STRUCTURE_CONFLICT", List.of());
    }
    ...
    private ResponseEntity<ErrorResponse> error(HttpServletRequest request, int status, String code,
        ...
        String message = switch (status) {
            ...
            case 409 -> "Program structure conflicted with persisted state.";
